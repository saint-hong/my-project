# 선형회귀 분석을 사용한 Boston Housing Data 집값 예측 - "집값에 영향을 미치는 가장 큰 요인은 무엇일까?"
...
- **r2 성능이 0.8대로 크게 낮아졌다.**
    - 주성분 25개의 유의확률도 대부분 0으로 측정되었다. 대부분의 독립변수가 종속변수에 중요한 변수라는 의미이다.
    - 자크베라 검정 유의확률이 0.99이므로 잔차는 확정적으로 정규분포를 따른다고 볼 수 있다.
...
- **14번의 모델링 중 f6_result_2(m_10) 모델의 성능이 0.911350으로 가장 높다.**
...
- **성능이 가장 높은 m_10은 3차 아웃라이어 제거와 모든 독립변수의 비선형 변형이 적용된 모델이다.(r2=0.911350)**
...
- 최종 모델의 예측 가중치에 따른 집값 변화량
    - **집값 상승에 영향을 미치는 요인**
        - RAD(방사형 고속도로의 접근성 지수)는 집값을 22,266 달러 상승시킨다.
        - RM(평균 방의 갯수)는 집값을 14,588 달러 상승시킨다.
        - B(도시별 흑인 비율 지수)은 집값을 1,719 달러 상승시킨다.
    - **집값 감소에 영향을 미치는 요인**
        - LSTAT(낮은 인구밀도 수치)는 집값을 3,103 달러 감소시킨다.
        - DIS(고용 센터까지의 가중거리 지수)는 집값을 2,781 달러 감소시킨다.
        - PTRATIO(교사 비율 지수)는 집값을 1,647 달러 감소시킨다.
    - **집값에 영향을 미치지 않는 요인**
        - ZN, NOX는 집값에 아무런 영향을 미치지 않는다. 
...
- **최종 모델 m_10의 독립변수별 예측 가중치 측정**
    - 유의확률이 유의수준(0.1)보다 높은 것은 제외한 후, 음,양의 상관관계를 합산하여 측정
    - 예측 가중치값에 1000을 곱하여 달러로 환산
