...
<p align=center> <img src = "./images/boston_city.jpg" height="75%"/> </p>
...
- 산점도의 기울기가 급격하게 증가하는 m_2와 m_3 구간은 변수의 중요도가 낮고 상관관계가 높은 독립변수 LSTAT와 DIS를 비선형 변형한 지점이다. 또한 두 독립변수는 종속변수와의 비선형 관계가 상대적으로 더 뚜렷하다. 두 독립변수를 비선형 변형함으로써 잔차의 분산(RSS)가 크게 줄어들어 성능(r2) 또한 크게 개선 된 것으로 보인다.
...
- m_11에서 m_pca4는 데이터의 갯수를 강제로 줄인(VIF, PCA) 모델에 해당하며 m_10 모델보다 성능이 점차 줄어드는 것을 볼 수 있다.
...
### 분석목적 1 : 가설과 회귀 분석을 통해서 얻은 지식의 차이를 비교한다.

#### 가설 1의 검증: 특징 데이터에 대한 경험적 지식
- **CRIM : 일치 **
...
#### 가설 2의 검증 : 집값을 증가, 감소 시키는 가장 큰 요인 3가지
- **집값을 증가시키는 요인**
    - 가설 : 1위 RM, 2위 CHAS, 3위 ZN
    - 분석 : 1위 RAD, 2위 RM, 3위 B
- **집값을 감소시키는 요인**
    - 가설 : 1위 AGE, 2위 INDUS, 3위 CRIM   
    - 분석 : 1위 LATAT, 2위 DIS, 3위 PTRATIO

#### 가설 3의 검증 : 선형회귀 모델을 통한 회귀 분석의 결과는 경험적 지식과 어떻게 다를까?
- 가설 : 경험적 지식과 완전히 일치하지는 않을 것이다. 데이터에 시대적, 환경적 특성이 반영되어 있기 때문이다.
- 분석 : 가설 1과 가설 2를 분석결과와 비교해 보면 일치하는 것과 불일치하는 것이 발생한다. 특히 ZN과 NOX 독립변수는 분석결과에 의하면 집값에 영향을 미치지 않는 것으로 나온다. PTRATIO와 B 독립변수는 가설과 분석결과가 불일치한다. 또한 집값을 증가시키는 요인으로 가설과 달리 분석결과는 방사형 도로의 접근성, 평균 방의 갯수, 흑인 인구 비율이 높은 것으로 나타난다. 집값을 감소시키는 요인으로는 인구밀도와 고용센터와의 거리, 도심내 교사의 비율이 크게 작용하는 것으로 나타난다. 즉 경험적 지식과 분석적 지식은 완전히 일치하지 않으며 어떤 결과는 경험적 지식과 완전히 정반대이기도 하다.

### 분석목적 2 : 회귀 모델을 사용하여 데이터로부터 예측값을 측정한다.
- <프로젝트 결과 1>의 7-2 예측값-실제값 분포도에서 분포의 형태가 기울기 1인 직선에 근접하게 나타나는 것을 볼 수 있다.
- **이를 통하여 성능이 가장 높은 m_10 모델의 예측값은 실제값과 유사하다는 것을 알 수 있다.**

### 분석목적 3 : 선형회귀 분석 방법을 사용하여 회귀 모델의 성능을 개선하기 위한 모델링을 적용한다.
- Boston Housing Data를 OLS 회귀분석 패키지를 사용하여 총 14번의 모델링을 시도하였고 가장 높은 성능은 0.911이 나왔다.
- 독립변수의 비선형 변형과 아웃라이어 제거 등을 통하여 모델의 성능을 개선시켰다.
- 비선형 변형으로 다항회귀 모델의 차수가 증가했지만 과최적화가 발생하지 않았으므로 모델과 데이터의 적합성이 확보된 것으로 보인다.
- 따라서 과최적화를 개선하기 위한 VIF, 정규화, PCA 방법을 적용하여도 모델의 성능이 개선되지는 않았다.
- 즉 데이터 전처리, 비선형 변형, 아웃라이어 제거 등을 적용하는 것으로도 모델의 성능을 높이는 것은 가능하다는 것을 알 수 있었다.
