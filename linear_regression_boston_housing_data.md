...
- Boston Housing Data를 최소자승법을 사용한 OLS 패키지를 사용하여 총 14번의 모델링을 시도하였고 가장 높은 성능은 0.911이 나왔다.
...
- **또한 어떤 현상에 관한 경험적 지식과 그 현상을 수치화한 데이터의 분석결과가 일치하지 않을 수 있지만 이를 조화롭게 활용할 수 있을 것이라는 알 수 있었다.** 일반적으로 과학적 분석의 결과를 좀 더 신뢰하고 경험적 지식을 배제할 수 있겠지만 오히려 경험적 지식과 분석적 지식의 "불일치"를 통해서 사업의 성과를 개선할 수 있을 것이다. 예를들면 집값을 감소시키는 요인에 대해 경험적 지식에 의해 INDUS(산업부지의 비율)를 선택했지만, 데이터 분석 결과 INDUS가 집값을 증가시키는 요인으로 나온 경우이다. 이러한 불일치는 우선 환경적 또는 시의적인 측면에서 고려해 볼 수 있다. 경험적 지식으로는 집 주변에 산업부지의 비율이 높아 산업시설이 생기면 집값이 떨어질 것으로 생각했지만, 도시의 산업이 발달하면서 노동자가 많이 유입되는 상황이라면 산업시설 근처에 위치한 집일 수록 집값은 높아질 수 있다는 것이다. 그렇다면 사업의 서비스 역량을 산업부지로 집값이 실제로 감소하는 지역과 산업부지로 집값이 실제로 증가하는 지역을 구분하여 차별화하거나 집중시킴으로써 사업의 성과를 달성할 수 있을 것이다.

### 프로젝트 과정에서 아쉬웠던 점
- 아래와 같은 문제들을 해결하지 못한 것을 들 수 있다. 프로젝트를 개선할 때 해결해보면 좋을 것 같다.
...
### 개선하면 좋을 점
- **회귀분석 모델로 OLS 패키지 외에 다른 회귀분석 패키지들을 연구하고 분석에 적용하여 비교해 보고자 한다.**
- statsmodels
...
- **또한 여러 모델의 하이퍼파라미터 튜닝에 집중한 모델링을 접목해 보고자 한다.**
    - 정규화 방법, 앙상블 방법, 변수선택 방법 등 프로젝트에서 사용하지 못한 다른 기능들을 실험해 보고 싶다.     
